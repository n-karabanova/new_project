	 ...
	 <tr> 
		...
		<tr> 
			<td class="main-page-text" rowspan="4"> 
				<h3>Франция</h3>
				<img class="img-country" src="france-1.jpg"></img>
				<p>
					<?php
					  $mysqli = new mysqli("localhost", "nkarabanova", "Naty2307", "newSql");
					  mysqli_set_charset($mysqli, 'utf8');
					  $query = "select * from country where Code = 'FRA'";
					...
					<?php
					  $mysqli = new mysqli("localhost", "nkarabanova", "Naty2307", "newSql");
					  mysqli_set_charset($mysqli, 'utf8');
					  $query = "select * from countryinfo where  CountryCode = 'FRA' order by OrderText";
					  $result = $mysqli->query($query); // выполнение запроса
					  while($row = $result->fetch_array(MYSQLI_ASSOC)) {
					    echo "<p>".$row["Text"]."</p>";
					  }
					...
					<h2>Лувр (Louvre Museum)</h2>
					<img class="img-country" src="france-2.jpg"></img>
					<?php
					  $mysqli = new mysqli("localhost", "nkarabanova", "Naty2307", "newSql");
					  mysqli_set_charset($mysqli, 'utf8');
					  $query = "select * from countryattraction where  CountryCode = 'FRA' order by OrderText";
					  $result = $mysqli->query($query); // выполнение запроса
					  while($row = $result->fetch_array(MYSQLI_ASSOC)) {
					    echo "<p>".$row["Text"]."</p>";
					  }

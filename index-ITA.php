	 ...
	 <tr> 
		...
		<tr> 
			<td class="main-page-text" rowspan="4"> 
				<h3>Италия</h3>
				<img class="img-country" src="italy-1.jpg"></img>
				<p>
					<?php
					  $mysqli = new mysqli("localhost", "nkarabanova", "Naty2307", "newSql");
					  mysqli_set_charset($mysqli, 'utf8');
					  $query = "select * from country where Code = 'ITA'";
					...
					<?php
					  $mysqli = new mysqli("localhost", "nkarabanova", "Naty2307", "newSql");
					  mysqli_set_charset($mysqli, 'utf8');
					  $query = "select * from countryinfo where  CountryCode = 'ITA' order by OrderText";
					  $result = $mysqli->query($query); // выполнение запроса
					  while($row = $result->fetch_array(MYSQLI_ASSOC)) {
					    echo "<p>".$row["Text"]."</p>";
					  }
					...
					<?php
					  $mysqli = new mysqli("localhost", "nkarabanova", "Naty2307", "newSql");
					  mysqli_set_charset($mysqli, 'utf8');
					  $query = "select * from countryattraction where  CountryCode = 'ITA' order by OrderText";
					  $result = $mysqli->query($query); // выполнение запроса
					  while($row = $result->fetch_array(MYSQLI_ASSOC)) {
					    echo "<p>".$row["Text"]."</p>";
					  }
